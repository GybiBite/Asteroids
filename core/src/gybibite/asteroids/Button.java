...
public final class Button {
  ...
  /** The {@link ShapeRenderer} object to draw the outline. */
  ...
  /**
   * Clickable button object that can be set to perform any generic set of commands. The text of the
   * button will scale based on the width.
   *
   ...
   */
  public Button(int x, int y, int width, int height, String label, SpriteBatch sb,
      ShapeRenderer sr) {
    this.width = width;
    this.height = height;
    this.x = x;
    ...
    this.sb = sb;
    this.sr = sr;
    corners = new float[] {x - width / 2, y + height / 2, x + width / 2, y + height / 2,
        x + width / 2, y - height / 2, x - width / 2, y - height / 2};
    gly.setText(font, label);

    font.getData().setScale(
        Math.min(((float) (width * 0.8) / gly.width), (float) (height * 0.4) / gly.height));
  }
  ...
  /**
   * Run the action previously set for the button. Intended for internal use, but no harm will be
   * done by being used outside of the class.
   */
  ...
  /**
   * Renders the button to the screen using the previously passed ShapeRenderer and SpriteBatch.
   */
  public void render() {
  ...
  /**
   * Sets whether or not you can click on the button. If the button is disabled, the text will be
   * grayed out.
   *
  ...
  public String toString() {
    return "Button: " + label + "Position: (" + x + ", " + y + ")" + "Size: (" + width + ", "
        + height + ")";
  }
}
